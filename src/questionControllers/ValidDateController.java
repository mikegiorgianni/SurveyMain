...
public class ValidDateController extends QuestionOps<ValidDate> {
    ...
    @Override
    public String displayQuestion(boolean withAnswers, ValidDate question) {
        StringBuilder sb = new StringBuilder("Valid date: " + question.getQuestion());
        if (withAnswers) {
            LocalDate date = LocalDate.parse(question.getDate().trim(), df);
            sb.append(" : ").append(date.format(df));
        }
        return sb.append("\n").toString();
    }
